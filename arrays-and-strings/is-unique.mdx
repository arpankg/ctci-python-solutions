...
If the index is between 65 and 90 which is 'A' - 'Z', we increase the index by 32 to implicitly convert the capital letter to small letter. Because we don't care about capitalization here, we need to assign the same index to both upppercase and lowercase.
